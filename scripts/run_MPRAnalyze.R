...
run_differential_analysis <- function(pdna_matrix,
        ...
        act <- getAlpha(obj, by.factor = "treatment") %>%
                mutate(tf = rownames(.))
...
# # Now just update rownames and convert to matrix
# rownames(parsed_pdna) <- tmp_parsed_pdna$tf
# rownames(parsed_cdna) <- parsed_cdna$tf
# names(negative_controls) <- tmp_parsed_pdna$tf

# # Printing the parsed data
# # message("\n\n\n==== Parsed data")
# # print(parsed_pdna %>% as_tibble %>% head())
# # message("\n\n\n==== Parsed cDNA data")
# # print(parsed_cdna %>% as_tibble %>% head())


# parsed_pdna %>%
#         select(-negative_controls) %>%
#         as.matrix() -> pdna_matrix
# parsed_cdna %>%
#         select(-tf, -promoter, -negative_control) %>%
#         as.matrix() -> cdna_matrix
...
old_contrast <- paste0("ZZZ_", opt$contrast_condition)
old_reference <- paste0("000_", opt$reference_condition)
default_contrast <- opt$contrast_condition
default_reference <- opt$reference_condition

parsed_results %>%
        select(-statistic, -fdr, -promoter) %>%
        # Now, rename the old_contrast column to default_contrast
        rename_with(~ gsub(old_contrast, default_contrast, .)) %>%
        rename_with(~ gsub(old_reference, default_reference, .)) %>%
